...
const SYMBOL_CHAR_CODES = makeCharArray(33, 47)
  .concat(makeCharArray(58, 64))
  .concat(makeCharArray(91, 96))
  .concat(makeCharArray(123, 126));
...
function generatePassword(
  ...
  if (upperCaseChecked)
    availableChars = availableChars.concat(UPPERCASE_CHAR_CODES);
  if (lowerCaseChecked)
    availableChars = availableChars.concat(LOWERCASE_CHAR_CODES);
  ...
  for (let i = 0; i < rangeAmount; i++) {
    const singlePasswordChar =
      characterAmount[Math.floor(Math.random() * availableChars.length)];
